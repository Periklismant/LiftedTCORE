...
from unified_planning.model.walkers import ExpressionQuantifiersRemover, LinearChecker
...
class LiftedTCORE(engines.engine.Engine, CompilerMixin):
    ...
    def _compile(
        ...
        # print(f"Get initial values time: {time.time() - start_time}")
        # print(Init)
        C = []
        ...
        # start_time = time.time()
        # print("Getting monitoring atoms...")
        I_prime, F_prime = self._get_monitoring_atoms(
            env, C, new_problem
        )  # Init, new_problem)
        # print(f"Get monitoring atoms time: {time.time() - start_time}")
        ...
        for a in new_problem.actions:
            ...
            for c in C:
                # print(f"Compiling constraint: {c} into action {a.name}")
                # start_time = time.time()
                ...
                # print(
                # f"Time {time.time()-start_time} for compiling constraint: {c} into action {a.name}"
                # )
    ...
    def _gamma(self, env, literal, action):
        ...
        for eff in action.effects:
            ...
            unification_variables = [
                var.variable() for var in uni if var.is_variable_exp()
            ]
            for z_var in z_vars:
            ...
            for param in uni:
                ...
                # print(
                #    f"Param: {param}. In literal fluent args? {param in literal.args}. In effect fluent args? {param in eff.fluent.args}"
                # )
                if param.is_parameter_exp() and param.parameter() in x_vars:
                    cond = And(cond, Equals(param, uni[param]))
                elif param.is_variable_exp() and param in eff.fluent.args:
                    cond = cond.substitute({param: uni[param]})
                # constraint variable case
                elif param.is_variable_exp() and (
                    param in literal.args
                    or (literal.is_not() and param in literal.args[0].args)
                ):
                    cond = And(cond, Equals(param, uni[param]))
                # universal effect case
                else:
                    raise ValueError(
                        f"{param} is not an operator parameter or an effect variable"
                    )
            if len(z_free) > 0:
                for z_var in z_free:
    ...
    def _unification(self, eff_fluentexp, lit_fluentexp, x_vars):
        ...
        if eff_fluentexp.fluent() != lit_fluentexp.fluent() or len(
            eff_fluentexp.args
        ) != len(lit_fluentexp.args):
            return None
        xi = dict()
        for i in range(len(eff_fluentexp.args)):
            f1arg = eff_fluentexp.args[i]
            f2arg = lit_fluentexp.args[i]
            # print(f"Trying to unify {f1arg} and {f2arg}")
            # print(f"Is {f1arg} a variable? {f1arg.is_variable_exp()}")
            # print(f"Is {f2arg} a variable? {f2arg.is_variable_exp()}")
            # The ith arguments of the fluents are constants that are different from each other
            if f1arg.is_object_exp() and f2arg.is_object_exp() and f1arg != f2arg:
                # print(f"Args: {f1arg} and {f2arg} are different Objects")
                return None
            # The ith argument of the effect fluent is a parameter of the action, the ith argument of the literal fluent is an object, and, based on typing, the object can be assigned to the parameter.
            elif (
                ...
                # print(
                #    f"Parameter: {f1arg} can be unified with Constraint Object: {f2arg}"
                # )
                xi[f1arg] = f2arg
            # The ith argument of the effect fluent is a z variable, the ith argument of the literal fluent is an object, and, based on typing, the object can be assigned to the variable.
            elif (
                ...
                # print(
                #    f"Effect Variable: {f1arg} can be unified with Constraint Object: {f2arg}"
                # )
                xi[f1arg] = f2arg
            # The ith argument of the effect fluent is an object, the ith argument of the literal fluent is a quantified variable, and, based on typing, the object can be assigned to the variable.
            elif (
                f2arg.is_variable_exp()
                and f1arg.is_object_exp()
                and f1arg.object().type.is_subtype(f2arg.variable().type)
            ):
                # print(
                #    f"Quantified Variable: {f2arg} can be unified with Effect Object: {f1arg}"
                # )
                xi[f2arg] = f1arg
            # The ith argument of the effect fluent is a parameter, the ith argument of the literal fluent is a quantified variable, and, based on typing, the parameter can be assigned to the variable.
            elif (
                f2arg.is_variable_exp()
                and f1arg.is_parameter_exp()
                and (
                    f1arg.parameter().type.is_subtype(f2arg.variable().type)
                    or f2arg.variable().type.is_subtype(f1arg.parameter().type)
                )
            ):
                # print(
                #   f"Quantified Variable: {f2arg} can be unified with Action Parameter: {f1arg}"
                # )
                xi[f2arg] = f1arg
            # The ith argument of the effect fluent is a z variable, the ith argument of the literal fluent is a quantified variable, and, based on typing, the z variable can be assigned to the variable.
            elif (
                f2arg.is_variable_exp()
                and f1arg.is_variable_exp()
                and (
                    f1arg.variable().type.is_subtype(f2arg.variable().type)
                    or f2arg.variable().type.is_subtype(f1arg.variable().type)
                )
            ):
                # print(
                # f"Quantified Variable: {f2arg} can be unified with Effect Variable: {f1arg}"
                # )
                xi[f1arg] = f2arg
            # Is it possible for f2arg, i.e., an argument of the literal fluent, to have parameters? I guess that the only options are object and variable.
            elif (
                f1arg.is_parameter_exp()
    ...
    def _get_relevant_init_values(self, env, formula, problem):
        checker = LinearChecker(problem, env)
        expression_quantifier_remover = ExpressionQuantifiersRemover(env)

        ground_formula = expression_quantifier_remover.remove_quantifiers(
            formula, problem
        )
        # print(f"Before formula: {formula}")
        # print(f"After formula: {ground_formula}")

        _, fluents_pos, fluents_neg = checker.get_fluents(ground_formula)
        init_values = dict()
        # print(f"Formula: {ground_formula}")
        for f in fluents_pos | fluents_neg:
            # print(f"Getting init value of fluent: {f}")
            init_values[f] = problem.initial_value(f)
        # print(init_values)
        return init_values

    def _evaluate_constraint(self, env, constr, problem):  # init_values):
        if constr.is_sometime():
            init_values = self._get_relevant_init_values(env, constr.args[0], problem)
            return HOLD, constr.args[0].substitute(init_values).simplify()
        elif constr.is_sometime_after():
            init_values0 = self._get_relevant_init_values(env, constr.args[0], problem)
            init_values1 = self._get_relevant_init_values(env, constr.args[1], problem)
            return (
                HOLD,
                env.expression_manager.Or(
                    constr.args[1].substitute(init_values1),
                    env.expression_manager.Not(constr.args[0].substitute(init_values0)),
                ).simplify(),
            )
        elif constr.is_sometime_before():
            init_values = self._get_relevant_init_values(env, constr.args[1], problem)
            return (
                SEEN_PSI,
    ...
    def _get_monitoring_atoms(self, env, C, problem):  # Init, problem):
        monitoring_atoms = []
        ...
        for constr in C:
            if constr.is_always():
                # start_time = time.time()
                Init = self._get_relevant_init_values(env, constr.args[0], problem)
                if constr.args[0].substitute(Init).simplify().is_false():
                    raise UPProblemDefinitionError(
                        "PROBLEM NOT SOLVABLE: an always is violated in the initial state"
                    )
                # print(f"Always: check unsolvability time: {time.time() - start_time}")
            else:
                # start_time = time.time()
                type, init_state_value = self._evaluate_constraint(
                    env, constr, problem
                )  # , Init)
                # print(f"Evaluate constraint at init time: {time.time() - start_time}")
                ...
                if constr.is_sometime_before():
                    ...
                    # print(
                    #    f"Sometime-before: check unsolvability time: {time.time() - start_time}"
                    # )
